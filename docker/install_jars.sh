...
# PowerMock reaches into Mockito's internals, and those moved in Mockito 4, so
# taking the newest Mockito here would leave the mocking silently not working.
# It is excluded from the bump rather than left to chance.
readonly PINNED=org.mockito:mockito-core
...
# The pin is worth failing the build over. If a newer Mockito ever arrives here,
# the mocking stops working in a way a learner would read as their own mistake.
if ! ls "${JARS_DIR}"/mockito-core-3.*.jar > /dev/null 2>&1; then
  >&2 echo "Expected mockito-core 3.x, found:"
  >&2 ls "${JARS_DIR}"/mockito-core-*.jar
  exit 42
fi
